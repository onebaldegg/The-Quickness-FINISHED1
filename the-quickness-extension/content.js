// THE QUICKNESS - Content Script
// Main functionality for screenshot, hover capture, and quick notes
...
(function() {
  ...
  class TheQuicknessExtension {
    constructor() {
      this.isActive = false;
      this.mode = null; // 'screenshot', 'hover', 'note'
    ...
    startHoverMode() {
      console.log('Starting hover mode');
      this.mode = 'hover';
      this.isActive = true;
      this.showOverlay();
      this.enableHoverCapture();
      this.showToast('Hover over images or text to capture (ESC to cancel)');
    }
    ...
    handleHoverMouseOver(e) {
      ...
      // Skip if hovering over our own UI
      if (element.closest('.tq-overlay') || element.classList.contains('tq-hover-highlight')) {
        return;
      }
      
      // Check if element is capturable (image, text content, etc.)
    ...
    async captureHoveredElement() {
      ...
      try {
        ...
        if (isImage) {
          ...
          // Capture formatted text with styling
      ...
      return new Promise((resolve, reject) => {
        if (imgElement.complete && imgElement.naturalWidth > 0) {
          ...
          try {
            ...
            // Fallback to src URL if canvas fails (CORS issues)
            resolve({
              dataURL: null,
              src: imgElement.src,
              alt: imgElement.alt || '',
    ...
    async captureTextElement(textElement) {
      ...
      // Extract formatted text with basic styling preserved
    ...
    async savePDF(note) {
      ...
      saveText.innerHTML = '<div class="tq-loading"><div class="tq-spinner"></div> Saving...</div>';
    ...
    async generateAndSavePDF(note) {
      ...
      pdf.text('Source: ' + data.url, margin, yPosition);
      yPosition += 15;
